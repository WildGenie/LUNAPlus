...
// 만약 존재하지 않으면 루나 인스톨 폴더의 setting.cfg를 읽어온다.
		...
		while( file.ReadString( line ) )
		{
			...
			else if( ! _tcsicmp( _T( "local" ), token ) )
			{
				...
				else if( ! _tcsicmp( _T( "UPRN" ), local ) )
				{
					SetLocal( Local::TypeUPRN );

					CLog::Put( _T( "Local is UPRN" ) );
				}
				continue;
			}
		...
		{
			{
				...
				// 080912 LUJ, FTP 계정과 암호가 있을 때는 아래와 같이 경로를 작성해준다
				//				ftp://[계정]:[암호]@[주소]
				download.mPath			= _T( "ftp://patcher:luna@24.119.245.227/current/" );
				download.mVersionTag	= _T( "UPRN" );
				download.mVersionFile	= _T( "MHVerInfo_New.txt" );
			}

			{
				...
				// 080919 LUJ, 가입 웹페이지 주소 수정
				url.mJoin		= _T( "http://uprn.comule.com/" );
				url.mHomepage	= _T( "http://uprn.comule.com/" );
				url.mContent	= _T( "http://uprn.comule.com/" );
			}
			break;
		}
	}
