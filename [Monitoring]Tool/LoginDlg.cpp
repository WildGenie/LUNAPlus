...
void CLoginDlg::EnterIDPass()
{
	...
	{
		MessageBox( L"ID & Password Check!!", L"Error!" );
		return;
	}
	...
	strcpy( msg.sOper.strOperID, CW2AEX< MAX_PATH >( id ) );
	strcpy( msg.sOper.strOperPWD, CW2AEX< MAX_PATH >( pass ) );
...
void CLoginDlg::LoginError( int error )
{
	...
	case 0: // 이상 오류
		{
			MessageBox( L"DB Error!!\nNotify to Programmer.\nClose Program.", L"Error!" );
			OnCancel();
		}
		break;
	case 1: // 등록안된 아이피
		{
			MessageBox( L"Not Registed IP. Close Program.", L"Error!" );
			OnCancel();
		}
		break;
	case 2: // 등록안된 아이디
		{
			MessageBox( L"Not Registed ID.", L"Error!" );
			OnCancel();
		}
		break;
	case 3: // 비번 오류
		{
			MessageBox( L"Password is Wrong.", L"Error!" );
			OnCancel();
		}
		break;
	case 4:
		{
			MessageBox( L"Wrong ServerTool Version!!\nClose Program.", L"Error!" );
			OnCancel();
		}
		break;
	case 99:	// level
		{
			MessageBox( L"You Are Wrong Right To Use This Tool!!\nClose Program.", L"Error!" );
			OnCancel();
		}
		break;
	}
