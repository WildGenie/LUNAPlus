...
{	
	...
	{
		::MessageBox(NULL,"Please create a palette tile first.","Error",MB_OK);
		return FALSE;
	}
...
BOOL CTool::ExportHeightField(char* szFileName)
{
	if (!m_pHField)
	{
		MessageBox(NULL,"Height field is not found.","Error",MB_OK);
		return FALSE;
	}
...
// 타일 클릭했을 때. 타일 편집 데이터가 바뀌는 것을 볼 수 있겠다..
...
// 마우스 좌클릭시 각종 브러시에 따라서 색칠? 을 해주는 부분이다
// 이 녀석을 기준으로 언두/리두의 타이밍을 잡으면 되겠다.
// 단지, 세이브하는 데이터의 기준이나 시리얼라이즈 같은 것의 단위는 서로 다를 수 밖에 없겠다.
void CTool::OnLButtonDown(UINT nFlags, CPoint* point) 
{
	...
	case EDIT_MODE_TILE:
		//In
		{
			...
			*/
			...
			if(m_pSectionSplitter->bSectionInputMode)
				...
				OnClickTile(&ptCursor,nFlags);		/// 타일 클릭했을시. 언두 대상이다.
			//--
		...
		break;
	case EDIT_MODE_DRAW_LIGHTMAP:
		OnClickLightMap(&ptCursor,nFlags);			/// 라이트맵 그릴 때?
		break;
	case EDIT_MODE_DRAW_VERTEXCOLOR:
		{
			BOOL	bResult;
...
void CTool::OnClickLight(POINT* ptCursor,UINT nFlags)
{
	if (m_dwEditMode & EDIT_TYPE_CREATE)
	{
		if (!m_bMapBuild)
		{
			MessageBox(NULL,"After the map is built, you can make a light.","Error",MB_OK);
			return;
		}
		LIGHT_DESC	light;
...
void CTool::SaveMap(char* szFileName)
{
	...
	/// Map
	/// 스태틱 모델과 하이트필드를 저장한다.
	/// 맵 파일을 봤는데 한개씩만 지정하는 듯.

	if (!m_pStaticModel && !m_pHField)
	{
		MessageBox(NULL,"Both StaticModels and HFields were not found.","Error",MB_OK);
		return;
	}
	...
	if (!fp) ASSERT("CTool::SaveMap() - File Pointer not found." );
	...
	while (fscanf(fp,"%s",buf)!=EOF)
	{
		...
		if (!lstrcmp(buf,PID_GX_OBJECT))
		{
			...
			for (DWORD i=0; i<dwGXObjectsNum; i++)
			{
				...
				/// 여기서 dwFlag중에 Lock이 켜져있으면 아래쪽의 위치셋팅이 안먹어서 0,0,0 에 위치하게 된다
				...
				if (gxo)
				{
					...
					// 이제 Lock된 녀석들도 제대로 위치가 세이브됐을것이다
					// 다시 복구
					if (l_LockFlag)
					{
						m_pExecutive->LockTransform(gxo);
