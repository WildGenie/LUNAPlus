...
void CMy4DyuchiGXMapEditorView::OnSaveTtb() 
{
	...
	{
		MessageBox( "Please do not check MoveSpot Input Mode!", "Failed to save. ", MB_ICONSTOP );
	}
	else if( g_pTool->m_pStartSplitter->bSectionInputMode )
	{
		MessageBox( "Please do not check StartSpot Input Mode!", "Failed to save. ", MB_ICONSTOP );
	}
	else
	{
