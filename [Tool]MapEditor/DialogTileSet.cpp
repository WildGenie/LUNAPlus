...
{
	// TODO: Add your control notification handler code here
	if(MessageBox("Are you sure you want to really initialized?",NULL,MB_YESNO) != IDYES)
		return;
...
void CDialogTileSet::OnLoad() 
{
	...
	{
		MessageBox("The tile is not loaded to the pallet. Please load the first tile pallet.\n");
		return;
	}
...
void CDialogTileSet::AddTileSet(CTileSet* pSet)
{
	...
	{
		//Confirm
		if(MessageBox("Do you want to modify the TileSet?",NULL,MB_YESNO) != IDYES)
			return;
...
{
	LPCTSTR szFileName = g_pTool->GetCurrentTileSetFileName();
...
{
	for (int i = 0; i < CTileSetDisplayer::MAX_SPRITE_NUM; i++)
	{
