...
#pragma comment(lib, "../lib/4dyuchigxgfunc/for_sse_debug/SS3DGFunc.lib")
...
void CMainFrame::OnBuildMap() 
{
	...
	{
		::MessageBox( 0, "Please specify higher values.","ERRORS! :D", MB_OK);
		return	;
	}
...
void CMainFrame::OnExportTileTable() 
{
	...
	{
		MessageBox( "Please do not check Section Input Mode!", "Failed to save.", MB_ICONSTOP );
	}
	else if( g_pTool->m_pSpotSplitter->bSectionInputMode )
	{
		MessageBox( "Do not check MoveSpot Input Mode!", "Failed to save.", MB_ICONSTOP );
	}
	else
	{
...
{
	// TODO: Add your command handler code here
	MessageBox("Nothing given.", MB_OK);
}
...
{
	if (IDNO == MessageBox("Exiting the program. \n\n Are you sure you really want to close the program?", "Exit", MB_YESNO))
	{
		return;
	}
